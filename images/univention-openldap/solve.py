...
configRegistry = Registry({
        ...
        'ldap/k5pwd': True, # This is normally not in UCR, only something that evaluate to true in 31modules
        'ldap/limits': 'users time.soft=-1 time.hard=-1',
        'ldap/monitor': False, # Would be nice to see a working monitor backend
        'ldap/monitor/acl/read/groups/': '', # What is even a sensible default here?
        'ldap/pwd_scheme_kinit': True,
        'ldap/master/port': '7389',
        'ldap/master': 'ucs-6045.' + args.domainname,
        'ldap/maxopenfiles': '8192',
        'ldap/overlay/lastbind': False, # TODO Add lastbind.la to image
        ...
        'ldap/policy/cron': '5 * * * *',
        'ldap/ppolicy': False, # TODO See if this has dependencies
        'ldap/ppolicy/enabled': False,
        'ldap/ppolicy/default': '',
        'ldap/server/ip': '127.0.0.1',
        'ldap/server/name': 'ucs-6045.' + args.domainname,
...
def warning_string():
    ...
    for f in sorted(listdir(path)): print('# \t' + path + f)

def resolve_variable(line):
    #VARIABLE_PATTERN = re.compile('@%@([^@]+)@%@')
    if '@%@ldap/debug/level@%@' in line:
        line = re.sub('@%@ldap/debug/level@%@', configRegistry['ldap/debug/level'], line)
    if '@%@hostname@%@' in line:
...
for line in sys.stdin:
    ...
    if line == '@!@\n':
        ...
        if inside_section:
            ...
            if (line == 'from univention.lib.misc import custom_groupname\n' or
                line == 'from univention.lib.misc import custom_username, custom_groupname\n'):
